...
cost, number_of_channels, pe_s = 0.1, 3, np.array([0.1, 0.4, 0.7])
...
def bisect(low, high, tol, max_confidence):
    ...
    while not converged or confidence < max_confidence:
        mid = (low + high) / 2
        ps = np.array([1 / (1 + (2 ** (mid / (1 - _pe_s)))) for _pe_s in pe_s])
        ps[ps < 0] = 0
        ps[ps > 1] = 1
...
cap, pxs = bisect(1e-4, 1e4, 1e-8, 10)
...
print(f'[INFO] CostConstrainedParallelBEC empirical: Channel Capacity Maximization for parallel BECs | '
      ...
      f'Achieving Bernoulli Parameters = {empirical_pxs}.')
